...
class foreman_proxy::plugin::ansible (
  ...
  ~> file { "${::foreman_proxy::dir}/.ansible":
    ensure => link,
    target => "${::foreman_proxy::var_dir}/ansible",
  }

  file { "${::foreman_proxy::var_dir}/ansible_galaxy":
    ensure => 'directory',
    owner  => $::foreman_proxy::user,
    group  => $::foreman_proxy::user,
  }
  ~> file { "${::foreman_proxy::dir}/.ansible_galaxy":
    ensure => link,
    target => "${::foreman_proxy::var_dir}/ansible_galaxy",
  }
